...
export default ({
  ...
  marginTop: marginTop_override
}) => {
  let ico = SuccessDialogIcon;
  let color = color_override ?? palette.success.dialog;
  if (error) {
    ico = ErrorDialogIcon;
    ...
    >
      <DialogTitle id="alert-dialog-title" style={{ overflow: 'visible' }}>
        <div style={{ color, justifyContent: 'center', display: 'flex' }}>
          <Icon
            style={{ marginTop: marginTop_override ?? -40, overflow: 'visible' }}
            icon={ico}
            width={80}
            height={80}
          />
      ...
      <DialogContent>
        <DialogContentText id="alert-dialog-description">
          <div style={{ color: 'black', textAlign: 'center' }}>{children}</div>
        </DialogContentText>
      </DialogContent>
      <DialogActions style={{ padding: 10 }}>
        {buttonText2 && (
          <Button disabled={disabledButton2} onClick={handleButton2} variant="outlined" autoFocus>
            {buttonText2}
          </Button>
        )}
